...
class BaseHeadSplit(nn.Module):
    def __init__(self, args, cid):
        ...
        if self.base.__class__.__name__ == 'ShuffleNetV2':
            head = self.base.fc
            if args.dataset == 'FashionMNIST':
                self.base.conv1[0] = nn.Conv2d(1, 24, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
            self.base.fc = nn.AdaptiveAvgPool1d(args.feature_dim)
            self.head = nn.Linear(args.feature_dim, args.num_classes)
        elif self.base.__class__.__name__ == 'EfficientNet': # original dim: 1280
            head = self.base.classifier
            if args.dataset == 'FashionMNIST':
                self.base.features[0][0] = nn.Conv2d(1, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
            self.base.classifier = nn.AdaptiveAvgPool1d(args.feature_dim)
            self.head = nn.Sequential(
                nn.Dropout(p=0.2, inplace=True),
                nn.Linear(args.feature_dim, args.num_classes)
            )
        elif self.base.__class__.__name__ == 'SqueezeNet':
            head = self.base.classifier
            self.base.classifier = nn.AdaptiveAvgPool2d(args.feature_dim, args.feature_dim)
            self.head = nn.Sequential(
        ...
        elif self.base.__class__.__name__ == 'MobileNetV2':
            head = self.base.fc
            if args.dataset == 'FashionMNIST':
                self.base.features[0][0] = nn.Conv2d(1, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
            self.base.fc = nn.AdaptiveAvgPool1d(args.feature_dim)
            if args.algorithm == 'FedNH':
                temp = nn.Linear(args.feature_dim, args.num_classes, bias=False).state_dict()['weight']
                self.head = nn.Parameter(temp)
    ...
    def forward(self, x):
        out = self.base(x)
        if not self.args.algorithm == 'FedNH':
            out = self.head(out)
        return out
